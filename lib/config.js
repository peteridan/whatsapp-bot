...
async function fetchGroqChatModels() {
            ...
            res.on('end', () => {
                try {
                    ...
                    const models = (data.data || [])
                        .filter((m) => /chat|instruct|completion|oss/i.test(m.id))
                        .map((m) => m.id)
                        .sort();
                    resolve(models);
                } catch {
...
async function resolveGroqModel() {
    ...
    const models = await fetchGroqChatModels();
...
async function getGroqModel() {
    ...
    resolvedGroqModel = await resolveGroqModel();

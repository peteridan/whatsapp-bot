...
async function handleCommandMessage(context) {
    ...
    if (!command) {
        console.log('DEBUG unknown command', resolvedCommand, 'in', context.chatId, 'from', context.senderId);
        await context.reply("Unknown command. Send !menu to see what's available.");
        return;
    }

    console.log('DEBUG command', resolvedCommand, 'in', context.chatId, 'from', context.senderId, 'ownerOnly=', ownerOnlyCommands.has(resolvedCommand), 'isOwner=', isOwner(context));

    if (ownerOnlyCommands.has(resolvedCommand) && !isOwner(context)) {
    ...
    try {
        ...
        if (message.includes('Socket not ready')) {
            await context.reply('Bot is not connected yet. Please wait or scan the QR code.');
            return;
        }
        if (commandName.toLowerCase() === 'ask' && message.includes('groq_429')) {
            await context.reply('Groq rate limit or free quota is exhausted right now. Please try again shortly.');
            return;
        }
        if (commandName.toLowerCase() === 'ask' && message.includes('groq_503')) {
        ...
        await context.reply(`Something went wrong: ${short}`);
    }
}
...
const commands = {
    ...
    ask: {
        run: async (context, args) => {
            ...
            const reply = await ai.generateAiReply(context.chatId, args.join(' '), context.senderName);
        ...
        run: async (context) => {
            ...
            try {
                ...
                const lines = metadata.participants.map((p) => {
                    const name = p.name || utils.getBaseUserId(p.id);
                    const presence = state.presence.get(p.id);
                    let status = '';
                    if (presence === 'available') {
                        status = ' 🟢';
                    } else if (presence === 'unavailable') {
                        status = ' ⚫';
                    } else if (presence === 'composing' || presence === 'recording') {
                        status = ' 🟡';
                    }
                    const admin = p.admin ? ` (${p.admin})` : '';
                    return `• ${name}${status}${admin}`;
                });
                const onlineCount = metadata.participants.filter((p) => state.presence.get(p.id) === 'available').length;
                const text = `Online (${onlineCount}/${metadata.participants.length}):\n` + lines.join('\n');
        ...
        run: async (context) => {
            ...
            try {
                ...
                const admins = metadata.participants
                    ...
                    .map(p => p.name || utils.getBaseUserId(p.id));
    ...
    promote: {
        run: async (context, args) => {
            ...
            const target = utils.resolveTargetJid(context, args);
            if (!target) {
                await context.reply('Usage: !promote <phone> (or @mention the user)');
                return;
            }
            try {
                await state.sock.groupParticipantsUpdate(context.chatId, [target], 'promote');
                const name = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
                await context.reply(`Promoted ${name} to admin.`);
            } catch (error) {
    ...
    demote: {
        run: async (context, args) => {
            ...
            const target = utils.resolveTargetJid(context, args);
            if (!target) {
                await context.reply('Usage: !demote <phone> (or @mention the user)');
                return;
            }
            try {
                await state.sock.groupParticipantsUpdate(context.chatId, [target], 'demote');
                const name = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
                await context.reply(`Demoted ${name} from admin.`);
            } catch (error) {
    ...
    add: {
        run: async (context, args) => {
            ...
            try {
                ...
                const name = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
                await context.reply(`Added ${name} to the group.`);
            } catch (error) {
    ...
    addbulk: {
        run: async (context, args) => {
            ...
            if (!raw) {
                await context.reply('Usage: !addbulk <phone1> <phone2> ...');
                return;
            }
            const numbers = raw.split(/[\s,]+/).map((n) => n.trim()).filter(Boolean);
            if (!numbers.length) {
                await context.reply('Usage: !addbulk <phone1> <phone2> ...');
                return;
            }
            ...
            for (const number of numbers) {
                const target = utils.resolveTargetJid(context, [number]);
                if (!target) {
                    failed.push(number);
                    continue;
                }
            ...
            await context.reply(lines.join('\n') || 'Could not add anyone. Make sure the bot is admin.');
        }
    },
    creategroup: {
        run: async (context, args) => {
    ...
    kick: {
        run: async (context, args) => {
            ...
            const target = utils.resolveTargetJid(context, args);
            if (!target) {
                await context.reply('Usage: !kick <phone> (or @mention the user)');
                return;
            }
            try {
                await state.sock.groupParticipantsUpdate(context.chatId, [target], 'remove');
                const name = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
                await context.reply(`Removed ${name} from the group.`);
            } catch (error) {
    ...
    warn: {
        run: async (context, args) => {
            ...
            const target = utils.resolveTargetJid(context, args);
            if (!target) {
                await context.reply('Usage: !warn <phone> (or @mention the user)');
                return;
            }
            ...
            const targetName = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
            await context.send(
                `🚫 @${utils.getBaseUserId(target)} (${targetName}), you have been warned. (Warning ${warnCount}/3)`,
                { mentions: [target] }
            );
            if (warnCount >= 3) {
                try {
    ...
    warnings: {
        run: async (context, args) => {
            ...
            const target = utils.resolveTargetJid(context, args);
            if (!target) {
                await context.reply('Usage: !warnings <phone> (or @mention the user)');
                return;
            }
            const warnKey = `${context.chatId}:${utils.getBaseUserId(target)}`;
            const count = state.store.userWarnings[warnKey] || 0;
            const name = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
            await context.reply(`${name} has ${count} warning(s).`);
        }
    },
    clearwarns: {
        run: async (context, args) => {
            ...
            const target = utils.resolveTargetJid(context, args);
            if (!target) {
                await context.reply('Usage: !clearwarns <phone> (or @mention the user)');
                return;
            }
            const warnKey = `${context.chatId}:${utils.getBaseUserId(target)}`;
            state.store.userWarnings[warnKey] = 0;
            utils.saveStore();
            const name = await utils.getDisplayNameForJid(state.sock, target, context.chatId);
            await context.reply(`Warnings cleared for ${name}.`);
        }
    },
    autopromo: {
        run: async (context, args) => {
    ...
    react: {
        run: async (context, args) => {
            if (!args.length) {
                await context.reply('Usage: reply to a message with !react <emoji>');
                return;
            }
            const emoji = args[0];
            const quotedInfo = context.rawMessage.message?.extendedTextMessage?.contextInfo;
            let targetKey = context.rawMessage.key;
            if (quotedInfo?.stanzaId) {
                targetKey = {
                    remoteJid: context.chatId,
                    id: quotedInfo.stanzaId,
                    fromMe: utils.getBaseUserId(quotedInfo.participant || '') === utils.getBaseUserId(state.sock?.user?.id || ''),
                    ...(context.isGroup ? { participant: quotedInfo.participant } : {})
                };
            }
            try {
                await state.sock.sendMessage(context.chatId, { react: { text: emoji, key: targetKey } });
            } catch (error) {
    ...
    search: {
        run: async (context, args) => {
            ...
            try {
                ...
                if (!res.ok) {
                    await context.reply('Search service returned an error.');
                    return;
                }
                const html = await res.text();
                const $ = cheerio.load(html);
                const results = [];
                $('.result__a, .result__snippet, .result__url').each((i, el) => {
                    if (i < 8) {
                        const text = $(el).text().trim();
                        const href = $(el).attr('href') || '';
                        if (text && href && !results.some(r => r.includes(text.slice(0, 40)))) {
                            results.push(text + '\n' + href);
                        }
                    }
                });
                ...
                await context.reply(results.slice(0, 5).join('\n\n'));
            } catch (error) {
                console.error('Search failed:', error);
                await context.reply('Search failed. Try again later.');
            }
        }
    },
    img: {
    ...
    play: {
        run: async (context, args) => {
            await media.playSong(context, args);
        }
    },
    toimg: {
        run: async (context) => {

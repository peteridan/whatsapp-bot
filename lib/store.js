...
function firebaseRequest(dbUrl, pathSegments, method, body, token) {
    return new Promise((resolve, reject) => {
        const pathname = '/' + [...pathSegments, 'bot-data.json'].filter(Boolean).join('/');
...
async function loadFirebaseStore() {
    try {
        ...
        const data = await firebaseRequest(dbUrl, [], 'GET', null, token);
...
async function saveFirebaseStore(payload) {
    try {
        const { dbUrl, token } = getFirebaseConfig();
        if (!dbUrl) return;
        await firebaseRequest(dbUrl, [], 'PUT', { value: payload, seen: true }, token);
    } catch (error) {

...
async function hasInternetConnection() {
    ...
    const outFile = isWin ? 'NUL' : '/dev/null';

    try {
        execSync(`${curl} -s -o ${outFile} -w "%{http_code}" https://www.gstatic.com/generate_204`, {
            timeout: 5000,
            stdio: ['pipe', 'pipe', 'pipe']
        });
        return true;
    } catch {
        // fallback to Node https if curl is unavailable or blocked
    }
    ...
    try {
        const result = await new Promise((resolve, reject) => {
            ...
            const req = lib.request('https://www.gstatic.com/generate_204', { method: 'HEAD', timeout: 5000 }, (res) => {
...
function formatMenu() {
    const sections = [
        { title: 'General', items: [
            ...
            '!online - List online members with presence',
        ...
        { title: 'Group Management (admins)', items: [
            ...
            '!addbulk <phone1> <phone2> ... - Add multiple numbers to the group',
            ...
            '!kick <phone|@user> - Remove user from group',
        ...
        { title: 'User Commands', items: [
            ...
            '!react <emoji> - Reply to a message with this to react to it',
        ...
        { title: 'Media / Fun', items: [
            ...
            '!play <song name> - Search YouTube and get the song link',
...
function createContext(message) {
    ...
    const senderName = message.pushName || getBaseUserId(senderId);
...
// Resolves a target user JID from either a real WhatsApp @mention (contextInfo.mentionedJid)
// or a typed phone number / JID in args[0]. Returns null if nothing usable was given.
function resolveTargetJid(context, args) {
...
function getQuotedImage(context) {
    const quoted = context.rawMessage.message?.extendedTextMessage?.contextInfo?.quotedMessage;
    if (!quoted) return null;
...
async function getDisplayNameForJid(sock, jid, chatId) {
    if (!jid) return 'Unknown';
    try {
        if (chatId.endsWith('@g.us') && sock) {
            const metadata = await sock.groupMetadata(chatId);
            const found = metadata.participants.find(p => jidNormalizedUser(p.id) === jidNormalizedUser(jid));
            if (found?.name) return found.name;
        }
    } catch {
        // ignore lookup failures
    }
    return getBaseUserId(jid);
}

function hasFfmpeg() {
    try {
        const ffmpegPath = require('ffmpeg-static');
        execSync(`"${ffmpegPath}" -version`, { encoding: 'utf8', timeout: 5000 });
        return true;
    } catch {
        try {

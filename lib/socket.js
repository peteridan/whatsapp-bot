...
function scheduleQrWatchdog() {
    ...
    state.qrWatchdogTimer = setTimeout(() => {
        ...
        if (state.qrWatchdogRestarts === undefined) state.qrWatchdogRestarts = 0;
        if (state.qrWatchdogRestarts >= 3) {
            console.log('QR watchdog: too many restarts, giving up until next manual restart.');
            return;
        }
        state.qrWatchdogRestarts += 1;
...
async function handleMessages(messages) {
    for (const message of messages) {
        ...
        const isViewOnce = !!utils.getIncomingViewOnceMedia(message);
        if (isViewOnce && context.isGroup) {
            await media.viewonceBypass(message, context);
        } else if (state.store.autoBypassViewOnce && isViewOnce) {
            await media.viewonceBypass(message, context);
        }

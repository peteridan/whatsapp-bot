...
// play-dl needs a SoundCloud client ID registered before playdl.search/stream
// will work for SoundCloud - without this, any SoundCloud call throws trying
// to read client_id off an unset token. Fetch it once at startup.
let soundcloudReady = null;
function ensureSoundcloudToken() {
    if (!soundcloudReady) {
        soundcloudReady = playdl.getFreeClientID()
            .then((clientID) => playdl.setToken({ soundcloud: { client_id: clientID } }))
            .catch((error) => {
                console.error('Failed to set up SoundCloud client ID:', error.message)
                soundcloudReady = null; // allow retrying on the next !play call
                throw error;
            });
    }
...
// Optional YouTube cookie mitigation for the "Sign in to confirm you're not
// a bot" wall. Not a guaranteed fix - a heavily-flagged datacenter IP can
// still get challenged even with valid cookies - but it can help. Set the
// YOUTUBE_COOKIE env var to enable it; leave it unset to skip silently.
let youtubeTokenSet = false;
function ensureYoutubeToken() {
    if (!youtubeTokenSet) {
        const cookie = process.env.YOUTUBE_COOKIE;
        if (cookie) {
            playdl.setToken({ youtube: { cookie } });
        }
        youtubeTokenSet = true;
    }
}
...
async function viewonceBypass(message, context) {
    ...
    try {
        ...
        if (inner.stickerMessage) {
            if (chatId) await utils.sendMedia(chatId, { sticker: buffer }, sendOptions);
        } else if (inner.imageMessage) {
            if (chatId) await utils.sendMedia(chatId, { image: buffer, caption: inner.caption || undefined }, sendOptions);
        } else if (inner.videoMessage) {
            if (chatId) await utils.sendMedia(chatId, { video: buffer, caption: inner.caption || undefined }, sendOptions);
        } else if (inner.documentMessage) {
            if (chatId) await utils.sendMedia(chatId, { document: buffer, mimetype: inner.documentMessage.mimetype || 'application/octet-stream', caption: inner.caption || undefined }, sendOptions);
        } else if (inner.audioMessage || inner.pttMessage) {
            if (chatId) await utils.sendMedia(chatId, { audio: buffer, mimetype: 'audio/ogg; codecs=opus' }, sendOptions);
        } else {
            if (chatId) await utils.sendMedia(chatId, { document: buffer }, sendOptions);
        }

        if (ownerJid && chatId !== ownerJid) {
            try {
                if (inner.stickerMessage) {
                    await utils.sendMedia(ownerJid, { sticker: buffer }, sendOptions);
                } else if (inner.imageMessage) {
                    await utils.sendMedia(ownerJid, { image: buffer, caption: inner.caption || undefined }, sendOptions);
                } else if (inner.videoMessage) {
                    await utils.sendMedia(ownerJid, { video: buffer, caption: inner.caption || undefined }, sendOptions);
                } else if (inner.documentMessage) {
                    await utils.sendMedia(ownerJid, { document: buffer, mimetype: inner.documentMessage.mimetype || 'application/octet-stream', caption: inner.caption || undefined }, sendOptions);
                } else if (inner.audioMessage || inner.pttMessage) {
                    await utils.sendMedia(ownerJid, { audio: buffer, mimetype: 'audio/ogg; codecs=opus' }, sendOptions);
                } else {
                    await utils.sendMedia(ownerJid, { document: buffer }, sendOptions);
                }
            } catch {
        ...
        if (chatId && chatId !== senderId && context?.isGroup) {
            try {
                const senderName = context.senderName || utils.getBaseUserId(senderId);
                await utils.sendText(chatId, `🔓 View-once bypassed for @${utils.getBaseUserId(senderId)}`, {
                    mentions: [senderId]
                });
            } catch {

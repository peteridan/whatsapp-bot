...
async function playSong(context, args) {
    ...
    try {
        ...
        if (video.seconds && video.seconds > MAX_PLAY_DURATION_SECONDS) {
            await context.reply(`"${video.title}" is too long to send (${video.timestamp}). Try a shorter track.`);
            return;
        }
        ...
        fs.mkdirSync(tempDir, { recursive: true });
        const base = Date.now();
        inPath = path.join(tempDir, `play-in-${base}.m4a`);
        outPath = path.join(tempDir, `play-out-${base}.mp3`);
        ...
        await new Promise((resolve, reject) => {
            ...
            (async () => {
                try {
                    ...
                    stream.stream.on('error', rejectOnce);
        ...
        if (!downloadFailed) {
            ...
            await context.reply(
                ...
                `⏱️ ${video.timestamp}\n\n` +
                `▶️ ${video.url}\n\n` +
                `Could not download audio automatically. Open the link to listen.`
            );
        }
    } catch (error) {
        const message = String(error?.message || error || '');
        console.error('playSong failed:', message);
        if (message.includes('No audio formats')) {
            await context.reply('No downloadable audio was found for that track. Try another song.');
            return;
        }
        if (message.includes('statusCode') || message.includes('fetch') || message.includes('play-dl') || message.includes('ytdl')) {
            await context.reply('Could not reach YouTube right now. Try again in a moment.');
            return;
        }
